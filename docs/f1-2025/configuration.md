...
!!! note "The address badge is the public IP only"
...
With collectors running on live UDP, press **RECORD** once to arm booth recording for every rig. The control then reads **ARMED** with a count of the rigs actively writing files. See [Running an Event](managing-collectors.md#recording-telemetry) for the full lifecycle.
...
1. Open **Config**, set the rig count and an **Event Name**, and configure your [destinations](controller-config.md).
